#!/usr/bin/python
# Modified by Waltteri Koskinen for Tampere Formula Student
# 09.11.2019
# Changes:
#		V1.6:
#			Minor bug fixes
...
# 			Max Value in GEMS CSV import file is 2147483
#  Version:    1.0
#
# Changes:
#		V1.0
		...
		with open(output_filename, 'wb') as output_file:
			...
			if GPSSats_index != 0:
				...
				while True:
					try:
						row = reader.next()
						if row[GPSSats_index] == '':
							row[GPSSats_index] = 0
						if row[GPSSats_index] >= num_of_sats:
						...
						print "Try to lower minimum satellite value"
			...
			while True: # Runs until exception occurs
				...
				for i in range(len(current_row)):
					try:
						if i == Longitude_index or i == Latitude_index:
							current_row[i] = current_row[i] * to_rads
					except TypeError:
						if current_row[i] == '':
				...
				# print current_row[Battery_index]
